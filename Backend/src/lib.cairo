...
mod DeVote {
    ...
    impl DeVoteImpl of IDeVote<ContractState> {
        ...
        fn change_person_rol(
            ...
            let signerPerson = self.persons.entry(get_caller_address());
            signerPerson.role.read() == 'admin';
            {
                ...
                if person.role.read() == 0 {
                    self
                        .emit(
                            UnauthorizeEvent {
                                function_name: 'change_person_rol',
                                type_error: 'person does not exist',
